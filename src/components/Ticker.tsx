export default function Ticker() {
  ...
  return (
    <div
      className="flex overflow-hidden border-b border-surface-600 bg-surface-800 py-2"
      role="marquee"
      aria-label="Live exchange rates"
    >
      <div className="flex animate-marquee gap-8 whitespace-nowrap pr-8 hover:[animation-play-state:paused]">
        {[...pairs, ...pairs].map((pair, i) => (
          <span key={i} className="flex items-center gap-2 font-mono text-sm">
            <span className="text-text-primary">{pair.code}</span>
            <span className="text-text-secondary">{pair.rate}</span>
